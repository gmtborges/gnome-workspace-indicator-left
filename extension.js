...
class WindowPreview extends St.Button {
  ...
  constructor(window) {
    ...
    this._minimizedChangedId = this._window.connect(
      "notify::minimized",
  ...
  _updateVisible() {
    ...
    this.visible =
      this._window.get_frame_rect().overlap(workArea) &&
...
class WorkspaceThumbnail extends St.Button {
  ...
  _addWindow(window) {
    if (this._windowPreviews.has(window)) return;
...
class WorkspaceIndicator extends PanelMenu.Button {
  ...
  _updateThumbnailVisibility() {
    ...
    const useMenu = vertical || workspaceManager.n_workspaces > MAX_THUMBNAILS;
    ...
    Main.panel.set_offscreen_redirect(
      useMenu
  ...
  _updateMenuOrnament() {
    for (let i = 0; i < this._workspacesItems.length; i++) {
      this._workspacesItems[i].setOrnament(
        i === this._currentWorkspace
  ...
  _createWorkspacesSection() {
    ...
    for (; i < workspaceManager.n_workspaces; i++) {
      this._workspacesItems[i] = new PopupMenu.PopupMenuItem(
        this._labelText(i)
      );
  ...
  _onScrollEvent(actor, event) {
    ...
    let diff = 0;
    if (direction === Clutter.ScrollDirection.DOWN) diff = 1;
    else if (direction === Clutter.ScrollDirection.UP) diff = -1;
    else return;
...
function enable() {
  ...
  Main.panel.addToStatusArea("workspace-indicator-left", _indicator, 1, "left");
}
